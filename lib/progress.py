...
class Progress(object):
    ...
    def __init__(self, args, recorder):
        ...
        if not self.args.has_log: schedule.every(2).seconds.do(self.eta_calc)

    def calc_total(self, duration_ms):
        self.show_total = True
        self.total_tracks += 1
        self.total_duration += duration_ms
        self.total_size += self.recorder.audio_recorder.get_bitrate() * duration_ms #int(get_args().quality) / 8

    def eta_calc(self):
        # exponential moving average
        def calc_rate(rate, avg_rate, smoothing_factor):
            if avg_rate is None: return rate
            else: return (smoothing_factor * rate) + ((1.0 - smoothing_factor) * avg_rate)
            ...
            if self.stat_prev is not None:
                ...
                if rate > 0.00000001:
                    ...
                    if self.show_total:
                        ...
                        self.total_eta = calc(total_position, self.total_duration,
    ...
    def update_progress(self, num_frames, sample_rate):
        ...
        def output_what_fits(init, output_strings):
            ...
            for _str in output_strings:
                _len = len(_str)
                if w + _len >= (self.term_width - 1): return

                print_str(_str)
                w += _len

        # make progress bar width flexible
        if self.term_width < 70: prog_width = 10
        elif self.term_width < 100: prog_width = 40 - (100 - self.term_width)
        else: prog_width = 40
        ...
        pct = int(self.song_position * 100 // self.song_duration) if self.song_duration > 0 else 0
        x = int(pct * prog_width // 100)

        # don't move cursor on first update
        if self.show_total:
            if self.move_cursor: print(Cursor.UP(2))
            else: self.move_cursor = True
        ...
        if self.show_total:
            ...
            total_pct = int(total_position * 100 // self.total_duration) if self.total_duration > 0 else 0
